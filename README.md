...
- **托盘设置页扩展**：增加音乐预设、统一场景自动化、空闲最终覆盖、事件反馈、更新检查和诊断信息。
...
- 音乐页可直接绑定当前播放器 PID，并选择预设颜色、封面主色或封面配色；此功能不依赖场景自动化。
- 内置“通用”音乐预设，并支持最多 8 个自定义音乐预设。

### 自动化与诊断

- 自动化分为音乐程序、灯效程序和时间计划：有声音乐程序优先，灯效程序按前台匹配，时间计划作为后备。
- 音乐程序可从当前音频会话绑定，重启后按稳定进程身份自动跟踪新 PID；多个播放器同时有声时前台播放器优先，否则按规则顺序。
- 音乐颜色可使用预设、Windows 媒体会话封面主色或封面配色；无封面时自动回退预设。
- 程序音频会话电平用于隔离节奏亮度；自适应频段分析仍使用系统混音并在诊断中标明。
- 支持空闲降亮或关灯作为所有场景之后的全局最终覆盖。
- 敲字闪烁和 Windows 通知闪烁作为独立的全局事件反馈。
- 打字反馈已同时支持普通灯效和音乐模式，并可按程序继承、强制开启或关闭。
- 支持检查更新。
- 设置页提供服务状态、驱动 DLL、当前音频源、前台应用、当前命中场景、空闲覆盖、无效规则原因和更新状态。
- 托盘菜单可查看当前歌曲和场景，并快捷启停规则、绑定播放器、切换封面颜色来源及事件反馈。
- 默认每天自动检查更新；发现新版本时主动提示，并在托盘保留下载入口。
